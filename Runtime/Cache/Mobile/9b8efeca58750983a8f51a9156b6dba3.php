...
<div class="page container">
    <div class="jindu">
        ...
        <div class="stage">
            ...
            <span class="<?php if($act == 'chuzhi'){?>active<?php }?>"><a href="<?php echo U('case/deal_with', array('id' => $id, 'act' => 'chuzhi'));?>">协同处置</a></span><span class="solid">→</span>
    ...
    <div class="information">
        <div class="title <?php if($act == 'chuzhi'){?>active<?php }?>" data-toggle="collapse" href="#chuzhi"
             aria-expanded="<?php if($act == 'chuzhi'){?>true<?php }else{?>false<?php }?>"
             aria-controls="collapseExample">当前阶段：处置<span class="caret"></span></div>
        <form action="<?php echo U('Case/deal_with');?>" method="post">
            <div id="chuzhi" class="submit collapse <?php if($act == 'chuzhi'){?>in<?php }?>">
                <div class="li">
                    <div class="left">流程操作</div>
                    <div class="right">
                        <label>
                            <input type="radio" class="radio" name="management_status" value="1" <?php if($Clist['management_status'] == 1){?>checked<?php }?>>
                            <span class="radio-text">处置完成,提交指挥中心</span>
                        </label><br>
                        <label>
                            <input type="radio" class="radio" name="management_status" value="2"> <span class="radio-text">驳回</span>
                        </label>
                    </div>
                    <div style="clear: both;"></div>
                </div>
                <div class="li zhuchibeizhu">
                    <div class="left">处置备注</div>
                    <div class="right">
                        <div>
                            <?php
 $mange_list = unserialize($Clist['management_record']); if(is_array($mange_list)){ for($i=1;$i<=count($mange_list);$i++){ ?>
                            <textarea name="management_record[]"><?php echo $mange_list[$i-1]?></textarea>
                            <input type="button" value="提交" style="float:right;text-align:center;padding:0 0.5em"
                                   class="chuzhisave"/>
                            <?php }}else{?>
                            <textarea name="management_record[]"><?php echo $mange_list?></textarea>
                            <input type="button" value="提交" style="float:right;text-align:center;padding:0 0.5em"
                                   class="chuzhisave"/>
                            <?php }?>
                        </div>
                ...
                <div class="li">
                    <div class="left">处置人</div>
                    <div class="right">
                        <span class="span-bottom"><?php echo ($member); ?></span>
                        &nbsp;
                        <span class="span-bottom">处置日期</span>
                        <span class="span-bottom">
                            <?php if(!empty($Clist['deal_with_time'])){ echo $Clist['deal_with_time']; } else { echo date('Y-m-d', time()); }?>
                        </span>
                        &nbsp;
